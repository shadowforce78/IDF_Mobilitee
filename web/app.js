async function load() {
    try {
        ...
        const validItems = [];
        data.forEach((item) => {
            const valid = item.departures.filter(
                (d) => d.time && !isNaN(new Date(d.time).getTime())
            );
            if (valid.length > 0) {
        ...
        function createTable(items) {
            ...
            items.forEach((item) => {
                const lineColor =
                    ...
                    (item.departures &&
                        item.departures[0] &&
                        item.departures[0].lineColor) ||
                    "#888888";
                const times = item.departures
                    .slice(0, 2)
                    .map((d) => {
                        const min = Math.max(
                            0,
                            Math.floor((new Date(d.time) - new Date()) / 60000)
                        );
                        return (
                            ...
                            '"><span class="minutes">' +
                            min +
                            ' min</span><span class="hour">' +
                            "</span></div>"
                        );
                    })
                    .join("");
                html +=
                    '<tr style="border-left: 6px solid ' +
                    lineColor +
                    '">' +
                    '<td class="line-cell">' +
        ...
        const html =
            '<div class="column">' + createTable(singleColumnItems) + "</div>";
